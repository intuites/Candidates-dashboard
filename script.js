...
const EDGE_SHEET_URL =
  "https://script.google.com/macros/s/AKfycbzi22VaAvIn6NCvDISJ3tsfJEymmTQLKmDhPqyxU2u8f4zIDHPNZYZhRi_jmDkZ9kldYg/exec";
...
function fileToBase64(file) {
  return new Promise((resolve, reject) => {
    ...
    if (!fileBlob) {
      reject(new TypeError("Invalid file: expected Blob or File"));
      return;
    }
...
async function uploadFileToDrive(
  ...
  try {
    ...
    if (!normalizedFile) {
      throw new TypeError("Invalid file: expected Blob or File");
    }
    ...
    const uniqueNum =
      fileIndex > 0 ? `_${fileIndex}` : `_${fileUploadCounter++}`;
    const fileName = `${namePart}${titlePart}${uniqueNum}.${ext}`.replace(
      /[^a-zA-Z0-9_.-]/g,
      "_",
    );
...
function handleFileSelect(file, dropArea, previewEl, type) {
  ...
  if (type === "resume") {
    ...
    const exists = pendingResumeFiles.some(
...
function renderTable(rows) {
  ...
  console.log(
    "First 3 rows:",
    rows.slice(0, 3).map((r) => ({ id: r.Unique, name: r["Candidate Name"] })),
  );

  rows.forEach((row) => {
    const skillsRaw = row.Skills || "";
    const skills = skillsRaw
      ? encodeHTML(skillsRaw.substring(0, 50)) +
        (skillsRaw.length > 50 ? "..." : "")
      : "-";
...
async function removeFromAllTitleMaps(candidateId) {
  try {
    ...
    const { data: titleMaps, error } = await db.from("Title_Map").select("*");
    ...
    for (const tm of titleMaps) {
      ...
      if (currentIds.includes(candidateId)) {
        ...
        console.log(
          `Removing ${candidateId} from "${tm.title}". New IDs:`,
          newIds,
        );
        ...
        if (updateError) {
          ...
          console.log(
            `Successfully removed candidate ${candidateId} from title "${tm.title}"`,
          );
...
document.addEventListener("click", async (e) => {
  ...
  if (btn.classList.contains("btn-edit") && btn.closest("tr")) {
    ...
    // Show existing files
    if (data.Resume)
      showExistingFile(data.Resume, resumePreview, resumeDropArea, "resume");
    if (data.DL) showExistingFile(data.DL, dlPreview, dlDropArea, "dl");
    if (data["Visa Copy"])
      showExistingFile(data["Visa Copy"], visaPreview, visaDropArea, "visa");
...
if (confirmDeleteBtn) {
  confirmDeleteBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      syncToSheet("Email_Atm", "delete", { Unique: id }).catch((e) =>
...
const handleFormSubmit = async (e) => {
  ...
  submitBtn.innerHTML =
    '<span class="spinner" style="width:16px;height:16px;border:2px solid #fff;border-top-color:transparent;border-radius:50%;animation:spin 0.8s linear infinite;display:inline-block;"></span> Saving...';
  ...
  try {
    ...
    if (pendingDlFile) {
      if (dlPreview)
        dlPreview.innerHTML =
          '<div class="file-uploading"><span class="spinner"></span> Uploading DL...</div>';
    ...
    if (pendingVisaFile) {
      if (visaPreview)
        visaPreview.innerHTML =
          '<div class="file-uploading"><span class="spinner"></span> Uploading Visa...</div>';
      const uploadedUrl = await uploadFileToDrive(
        pendingVisaFile,
        candidateName,
        candidateTitle,
    ...
    if (editingId) {
      ...
      if (pendingResumeFiles.length > 0) {
        if (resumePreview)
          resumePreview.innerHTML =
            '<div class="file-uploading"><span class="spinner"></span> Uploading resume...</div>';
        const uploadedUrl = await uploadFileToDrive(
          pendingResumeFiles[0].file,
          candidateName,
          candidateTitle,
        );
        if (uploadedUrl && uploadedUrl !== "UPLOADED_CHECK_DRIVE") {
          ...
          resumeUrl = `[Uploaded: ${pendingResumeFiles[0].file?.name || "resume"}]`;
        }
      }
      ...
      const filesToProcess =
        pendingResumeFiles.length > 0
          ? pendingResumeFiles
          : [{ file: null, title: candidateTitle }];
      ...
      for (let i = 0; i < filesToProcess.length; i++) {
        ...
        if (file) {
          console.log(
            `Uploading resume ${i + 1}/${filesToProcess.length}: ${file.name} with title: "${resumeTitle}"`,
          );
          if (resumePreview) {
...
if (form) {
  ...
  console.error(
    "Form element not found! Check if id='candidateFormInner' exists in HTML",
  );
}
  ...
  filtered.forEach((t) => {
    ...
    const itemsHtml =
      ...
      `;
            ...
            .join("")
        : '<div class="muted">No candidates assigned</div>';
      ...
      <div id="multiList" style="max-height:350px;overflow:auto;border:1px solid #e2e8f0;border-radius:12px;background:#f8fafc;">
        ${
          ...
          `;
                ...
                .join("")
            : '<div style="padding:24px;text-align:center;color:#94a3b8;">No active candidates available. Mark candidates as Active first.</div>'
        }
      </div>
  ...
  if (multiSearch) {
    multiSearch.addEventListener("input", () => {
      ...
      multiList.querySelectorAll("label").forEach((lbl) => {
        lbl.style.display =
          !q || lbl.textContent.toLowerCase().includes(q) ? "flex" : "none";
      });
    });
  }

  document
    .getElementById("multiCancel")
    .addEventListener("click", () => modalBackTM.classList.add("hidden"));
